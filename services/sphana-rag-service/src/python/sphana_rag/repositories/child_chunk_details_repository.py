...
from sphana_rag.models import ChildChunkDetails
from sphana_rag.models import ListResults
from .base_documents_repository import BaseDocumentsRepository

@singleton
class ChildChunkDetailsRepository(BaseDocumentsRepository[ChildChunkDetails]):
    
    def __init__(self):
        self.__last_unique_id_map: dict[str, int] = {}
        self.__last_unique_id_lock = threading.Lock()
        db_location: str = "./.database/child_chunk_details_db" # TODO: take from env variables
    ...
    def upsert(self, shard_name: str, child_chunk_details: ChildChunkDetails) -> None:
        self._upsert_document(shard_name, child_chunk_details.child_chunk_id, child_chunk_details)

    def delete(self, shard_name: str, child_chunk_id: str) -> None:
        self._delete_document(shard_name, child_chunk_id)

    def read(self, shard_name: str, child_chunk_id: str) -> Optional[ChildChunkDetails]:
        return self._read_document(shard_name, child_chunk_id)
    
    def list(self, shard_name: str, offset: Optional[str], limit: int) -> ListResults[ChildChunkDetails]:
        return self._list_documents(shard_name, offset, limit)
    
    def exists(self, shard_name: str, child_chunk_id: str) -> bool:
        return self._document_exists(shard_name, child_chunk_id)
