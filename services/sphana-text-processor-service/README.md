...
1. Install Python Dependencies:<br>
   `uv sync --project .\services\sphana-text-processor-service`
2. Activate the Virtual Environment:<br>
   `.\services\sphana-text-processor-service\.venv\Scripts\activate`

## Refresh Dependencies
```
uv pip install -e .\services\sphana-text-processor-service\ --refresh --reinstall 
```

## Run Locally
```
uv run python .\services\sphana-text-processor-service\src\python\sphana_text_processor\
```
...
python -m nuitka `
   ...
   --output-dir=.\services\sphana-text-processor-service\target `
   --include-data-dir=.\services\sphana-text-processor-service\src\resources=resources `
   .\services\sphana-text-processor-service\src\python\sphana_text_processor
```
...
Run the following command:
1. No CUDA:
   `docker build --no-cache -t sphana.processor:1.0.0 -f .\services\sphana-text-processor-service\src\docker\DockerFile .`
2. CUDA 12.8 Support:
   `docker build --no-cache -t sphana.processor:1.0.0-cuda12.8 -f .\services\sphana-text-processor-service\src\docker\Dockerfile-CUDA12.8 .`
