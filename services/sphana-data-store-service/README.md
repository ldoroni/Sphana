...
1. Install Python Dependencies:<br>
   `uv sync --project .\services\sphana-data-store-service`
2. Activate the Virtual Environment:<br>
   `.\services\sphana-data-store-service\.venv\Scripts\activate`

## Refresh Dependencies
```
uv pip install -e .\services\sphana-data-store-service\ --refresh --reinstall 
```

## Run Locally
```
uv run python .\services\sphana-data-store-service\src\python\sphana_data_store\
```
...
python -m nuitka `
   ...
   --output-dir=.\services\sphana-data-store-service\target `
   --include-data-dir=.\services\sphana-data-store-service\src\resources=resources `
   .\services\sphana-data-store-service\src\python\sphana_data_store
```
...
Run the following command:
1. No CUDA:
   `docker build --no-cache -t sphana.store:1.0.0 -f .\services\sphana-data-store-service\src\docker\DockerFile .`
2. CUDA 12.8 Support:
   `docker build --no-cache -t sphana.store:1.0.0-cuda12.8 -f .\services\sphana-data-store-service\src\docker\Dockerfile-CUDA12.8 .`
