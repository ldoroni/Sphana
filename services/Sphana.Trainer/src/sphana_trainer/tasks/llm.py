...
from sphana_trainer.config import LlmConfig
from sphana_trainer.exporters.onnx import export_llm_model
...
class LlmTask(BaseTask):
    """Export LLM model to ONNX."""

    def __init__(self, config: LlmConfig, artifact_root: Path):
        self.config = config
        self.artifact_root = Path(artifact_root)

    def run(self) -> LlmTrainingResult:
        component_dir = Path(self.config.output_dir)
        run_dir = prepare_run_directory(component_dir, self.config.version)
        
        logger.info("Exporting LLM model {}...", self.config.model_name)
        
        onnx_dir = run_dir / "onnx"
        tokenizer = AutoTokenizer.from_pretrained(self.config.model_name)
        
        onnx_path, quantized_path = export_llm_model(
        ...
        logger.success("LLM artifacts saved to {}", run_dir)
