...
1. Install Python Dependencies:<br>
   `uv sync --project .\services\sphana-text-facade-service`
2. Activate the Virtual Environment:<br>
   `.\services\sphana-text-facade-service\.venv\Scripts\activate`

## Refresh Dependencies
```
uv pip install -e .\services\sphana-text-facade-service\ --refresh --reinstall 
```

## Run Locally
```
uv run python .\services\sphana-text-facade-service\src\python\sphana_text_facade\
```
...
python -m nuitka `
   ...
   --output-dir=.\services\sphana-text-facade-service\target `
   --include-data-dir=.\services\sphana-text-facade-service\src\resources=resources `
   .\services\sphana-text-facade-service\src\python\sphana_text_facade
```
...
Run the following command:
1. No CUDA:
   `docker build --no-cache -t sphana.text-facade:1.0.0 -f .\services\sphana-text-facade-service\src\docker\DockerFile .`
2. CUDA 12.8 Support:
   `docker build --no-cache -t sphana.text-facade:1.0.0-cuda12.8 -f .\services\sphana-text-facade-service\src\docker\Dockerfile-CUDA12.8 .`
