...
1. Install Python Dependencies:<br>
   `uv sync --project .\services\sphana-processor-service`
2. Activate the Virtual Environment:<br>
   `.\services\sphana-processor-service\.venv\Scripts\activate`

## Refresh Dependencies
```
uv pip install -e .\services\sphana-processor-service\ --refresh --reinstall 
```

## Run Locally
```
uv run python .\services\sphana-processor-service\src\python\sphana_processor\
```
...
python -m nuitka `
   ...
   --output-dir=.\services\sphana-processor-service\target `
   --include-data-dir=.\services\sphana-processor-service\src\resources=resources `
   .\services\sphana-processor-service\src\python\sphana_processor
```
...
Run the following command:
1. No CUDA:
   `docker build --no-cache -t sphana.processor:1.0.0 -f .\services\sphana-processor-service\src\docker\DockerFile .`
2. CUDA 12.8 Support:
   `docker build --no-cache -t sphana.processor:1.0.0-cuda12.8 -f .\services\sphana-processor-service\src\docker\Dockerfile-CUDA12.8 .`
